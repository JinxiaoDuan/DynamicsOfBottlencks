...
{ 
    ...
    void ReadSpeed(int day)
    {
		...
		char filename[100];
		if (date<10)		sprintf (filename,  "relativevelocity_5min_0%d.tntp", date);   	
		if (date>=10)	sprintf (filename,  "relativevelocity_5min_%d.tntp", date);   	
		...
		if (date<10)		sprintf (filename_miss,  "outlier_5min_0%d.tntp", date);   	
		if (date>=10)	sprintf (filename_miss,  "outlier_5min_%d.tntp", date);   	
	...
	{ 
		...
		char filename[100];
		if(date>=10)	sprintf (filename,"generated data/bottleneckdynamics_step=%d_%d.csv",step,date);
		if(date<10)		sprintf (filename,"generated data/bottleneckdynamics_step=%d_0%d.csv",step,date);
        ofstream trees(filename);
        trees << "linkID of bott." <<','<<"Size (num. of cong.)"<<',' << "time step" <<"\n";
	    ...
	    for(int i=1; i<=linkNum;i++)
	    {
	    	...
	    	if (is_element_in_vector(trunkidset,top)==1) 
			{
				...
				//{
					//treelinkid=orderlink[j]; 
					//trees << treelinkid <<',' <<treesizetemp <<','<< top <<"\n";					
				//}
				trees << top <<','<< treesizetemp <<','<< step <<"\n";	// top-1 if begins from id in 0
				order.push_back(treesizetemp); 
			} 
		}
		
		if (order.size()>0) maxig=order[0];
		if (order.size()==0) maxig=0;
	    
	    for(int i=0;i<order.size();i++)
	        if(order[i]>maxig) maxig=order[i];   
		cout<< endl; 
	    cout << "size of congestion:" <<  maxig <<endl; 
